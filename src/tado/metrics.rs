...
use super::model::{WeatherApiResponse, ZoneStateApiResponse};
...
pub fn set_zones(zones: Vec<ZoneStateApiResponse>) {
    for zone in zones {
        ...
        if let Some(setting_temperature) = zone.setting.temperature {
            ...
            SETTING_TEMPERATURE
                .with_label_values(&[zone.name.as_str(), "celsius"])
                .set(value);
            info!(
        ...
        // sensor temperature
        if let Some(inside_temperature) = zone.sensorDataPoints.insideTemperature {
            // celsius
        ...
        // sensor humidity
        if let Some(humidity) = zone.sensorDataPoints.humidity {
        ...
        // heating power
        if let Some(heating_power) = zone.heatingPower {
        ...
        // // ac power
        // if let Some(ac_power) = zone.activityDataPoints.acPower {
